...
from tensorflow.keras.optimizers.legacy import SGD      ####### change
from tensorflow.keras.models import load_model
import pandas as pd ######### change
...
class GenericAssistant(IAssistant):
    ...
    def train_model(self):
        ...
        # training = np.array(training)                                        ######### change
    ...
    def _predict_class(self, sentence):
        ...
        # print('**********probabilities(-1):', res)
        ERROR_THRESHOLD = 0.1
        results = [[i, r] for i, r in enumerate(res) if r > ERROR_THRESHOLD]
        # print('**********probabilities(0):', results)

        results.sort(key=lambda x: x[1], reverse=True)
        # print('**********probabilities(1):', results)
        return_list = []
        for r in results:
